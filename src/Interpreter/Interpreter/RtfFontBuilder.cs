...
using System.Text;
using Itenso.Rtf.Interpreter.Model;
using Itenso.Rtf.Interpreter.Support;
		...
		protected override void DoVisitTag( IRtfTag tag )
		{
			switch ( tag.Name )
			{
				...
				case RtfSpec.TagThemeFontBiMinor:
					...
					break;
				case RtfSpec.TagFont:
                    // Only Set the font id and index if there is a valid number.
                    // We might have bad rtf text with just \f with no index
                    if (!string.IsNullOrWhiteSpace(tag.ValueAsText))
                    {
                        fontId = tag.FullName;
